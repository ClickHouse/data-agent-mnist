"""
Evaluate candidates against the majority-vote ground truth.
...
def load_ground_truth(annot_path: Path = ANNOT_PATH) -> dict[str, dict]:
    """Usable ground truth keyed by trace_id (drops committee-excluded questions)."""
    gt = {}
...
def main():
    ...
    def score_pair(pair: tuple[str, str]) -> dict:
        ...
        return {
            ...
            "error": replay["error"],
            # Exact per-candidate token usage. None for cells written by
