"""
Contamination probe (AI-1233): is the benchmark in any model's training data?
...
# configuration so the probe travels; the defaults are the board's (AI-1858).
...
# AI-1858 used that wording, so a re-run is not strictly comparable to them.
ENTITY_PROMPT = (
...
def _complete(model_key: str, prompt: str, max_tokens: int = 300) -> str:
    ...
    if "qwen3p8-max" in model_id:
        ...
        # (starved 90%+ of cells even at 8192, AI-1801); the probe measures bare
    ...
    # empty generation — scoring it 0 would fabricate a clean probe (AI-1801).
    choice = resp.choices[0]
    msg = choice.message
