...
# importing this module constructs six clients. See registry.py (AI-1860).
...
# Overridable for sensitivity sweeps (AI-1820): the budget is never announced to
...
class _GCPAuth(httpx.Auth):
    ...
    developer machine does (AI-1862).
...
# gpt-5.6, so it takes the Responses-API runner (AI-1800).
...
class TokenUsage:
    ...
    appears in the transcript (AI-1540). That is exactly the set of models whose
    cost we most need.
...
def run_candidate_bedrock(
    ...
    # starvation AI-1490 fixed for the OpenAI-compatible path, which the Bedrock path
    # never got.
...
def run_candidate_openai_compat(
    ...
    # on plain prose (observed finish_reason=length in the pre-trust probe, AI-1801).
    ...
    sqls, sql_results = [], []
    served = model_id  # resolved id as reported by the endpoint (AI-1818)
...
def judge_token_budget(model_id: str, max_tokens: int) -> tuple[str, int]:
    ...
    two live votes to score at all (AI-1862).
...
def judge_panel(
    ...
    """Score a candidate with a provider-diverse panel (AI-997). Majority vote over
...
# ── Majority-vote ground truth (AI-997) ───────────────────────────────────────
...
def _link_columns(keys_a: list, keys_b: list) -> dict:
    ...
    try:
        ...
        # account, because this call was hardcoded to Bedrock (AI-1862). Once per
...
def majority_vote_gt(annotator_runs: dict) -> dict:
    """Build ground truth from independent annotator runs (AI-997).
...
def bt_sigma_aggregate(*args, **kwargs):
    ...
    (arXiv 2602.16610). Deferred per AI-997 — only worthwhile once the benchmark
    scales past ~71 questions. Use majority_vote_gt / judge_panel until then."""
    raise NotImplementedError(
        "BT-sigma jury deferred until the benchmark scales past ~71 questions (AI-997)"
    )
