"""Regenerate examples/saas/data/*.csv, the frozen fixture behind the example warehouse.
...
releases, and examples/saas/questions.jsonl names tenants directly, so a Faker upgrade
...
So: RUNNING THIS CAN INVALIDATE examples/saas/questions.jsonl. Any change to the seed,
...
rather than person names, because no invented person name is nobody's real name.
    ...
    uv run examples/saas/generate_example_data.py
"""
...
def main() -> None:
    ...
    print("\nIf the tenant names above changed, examples/saas/questions.jsonl is now "
