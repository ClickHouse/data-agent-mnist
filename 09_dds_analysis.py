...
`--layer-marker crm.` reproduces this analysis there.
...
def compute(dataset: Path = SYNTH, marker: str = LAYER_MARKER):
    ...
    # fm_labels.jsonl comes from 07_failure_modes and is OPTIONAL: it only feeds
    # the FM2 column. Required, it made the whole analysis unavailable to anyone
    # who had not run the labelling stage, which is every adopter on a first pass
    # and both example instances.
    fm = {}
    fm_path = dataset / "fm_labels.jsonl"
    if fm_path.exists():
        ...
        print(f"note: no {fm_path.name} in {dataset}, so the FM2 column will read 0. "
              f"Run 07_failure_modes.py to populate it.\n")
