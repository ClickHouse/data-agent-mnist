"""
Build majority-vote ground truth for the synthetic text2sql benchmark.
...
# Trace ids the schema-compat classifier wrongly rejects but which are answerable
# against the seeded schema, re-added by hand. The manual *exclusion* list this
# used to sit beside is gone: its job is now done by the all-annotators-disagree
# filter.
#
# From configuration, because the contents are a property of one corpus and not
# of the code. They were a literal here, which put two paraphrased production
# questions and the table that answers each into a file meant to be published,
# along with two trace ids that are keys into a private question set. The
# mechanism travels; the list does not.
#
# DAM_MANUAL_ADDITIONS names a JSON file of {"trace_id": "why it was re-added"}.
# The reason is for whoever reads the file later and is not used here.
def _load_manual_additions() -> dict:
    path = os.environ.get("DAM_MANUAL_ADDITIONS")
    if not path:
        return {}
    p = Path(path).expanduser()
    if not p.exists():
        raise SystemExit(f"DAM_MANUAL_ADDITIONS points at {p}, which does not exist")
    return json.loads(p.read_text())


MANUAL_ADDITIONS = _load_manual_additions()
...
def load_questions(classify: bool = True, path: Path | None = None) -> list[dict]:
    ...
    # path that cannot exist. DAM_QUESTIONS carries it for our own runs.
    ...
    if src is None:
        raise SystemExit(
            ...
            "  examples/saas/questions.jsonl is a worked set.")
    if not src.exists():
...
def main():
    ...
    ap.add_argument("--session-timezone", default=None,
                    help="pin the chDB session timezone. Needed for a DateTime "
                         "grain: values are rendered in the session zone, so the "
                         "snapshot date moves with the reader's locale")
