    ...
    def __init__(self, db_path: Path = DB_PATH,
        ...
        if not db_path.exists():
            raise RuntimeError(
                ...
                f"examples/saas/seed_example.py to build the small one that comes with "
                f"the harness.")
        ...
        # chDB parses AND renders DateTime in the session timezone, so a warehouse
        # with a DateTime grain answers differently depending on where it is read.
        # Measured on the same fixture: max(timestamp_hour) is
        # "2026-06-30 00:00:00" under UTC and "2026-06-29 17:00:00" under
        # America/Los_Angeles, which moves the snapshot date a full day and with it
        # every relative window the questions use.
        #
        # Opt-in, and unset by default: the board's ground truth was built without
        # it, so pinning a zone here would silently re-date that corpus. A fixture
        # meant to travel passes one.
        if session_timezone:
            self.sess.query(f"SET session_timezone = '{session_timezone}'")
        ...
        # check above says what to pass; this has to as well.
        try:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"examples/saas/schema.md is a worked one.") from e
